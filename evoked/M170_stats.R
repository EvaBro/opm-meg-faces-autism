...
df <- df %>%
  select(subject, dx, age, age_bin, trials, ffg_lat_l, ffg_amp_l, ffg_lat_r, ffg_amp_r, power_l, power_r, srs_total_T_merged, HM, num_bad_channels
  ) %>% rename(latency_l = ffg_lat_l, 
               latency_r = ffg_lat_r, 
               amplitude_l = ffg_amp_l, 
               amplitude_r = ffg_amp_r
  ) %>% 
  mutate(dx = factor(dx),
...
df_hemisphere <- df %>%
  pivot_longer(
    cols = c(latency_l, latency_r, amplitude_l, amplitude_r, power_l, power_r),
  ...
  select(subject, dx, age, age_bin, hemisphere, latency, amplitude, power, srs_total_T_merged, trials, HM, num_bad_channels
  ) %>% 
...
### POWER, aka mean signal magnitude relative to baseline
model_power_interaction <- lmer(power ~ dx * hemisphere + (1 | subject), data = df_hemisphere)
summary(model_power_interaction)

# The interaction term is non-significant so we let it go:
model_power_simple <- lmer(power ~ dx + hemisphere + (1 | subject), data = df_hemisphere)
summary(model_power_simple)

# Compare the models
anova(model_power_interaction, model_power_simple)

# Interaction term is not significant and does not improve model fit, so will move forward with simple model
eta_squared(model_power_simple, partial = TRUE)# Check effect size
r2(model_power_simple)
check_model(model_power_simple) # Check model assumptions
emmeans(model_power_simple, ~ dx) # Check adjusted group means
...
# POWER
srs_pow_interaction <- lmer(power ~ srs_total_T_merged * hemisphere + (1|subject), data = df_hemisphere)
summary(srs_pow_interaction)

srs_pow_simple <- lmer(power ~ srs_total_T_merged + hemisphere + (1|subject), data = df_hemisphere)
summary(srs_pow_simple)

anova(srs_pow_interaction, srs_pow_simple)

# Interaction not significant + does not improve model fit -> continue with simpler model
eta_squared(srs_pow_simple, partial = TRUE)# Check effect size
r2(srs_pow_simple)
check_model(srs_pow_simple) # Check model assumptions